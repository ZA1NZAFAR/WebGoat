...
public class SqlInjectionLesson10 extends AssignmentEndpoint {
  ...
  protected AttackResult injectableQueryAvailability(String action) {
    ...
    // Modified query to use a placeholder for the parameter
    String query = "SELECT * FROM access_log WHERE action LIKE ?";

    try (Connection connection = dataSource.getConnection()) {
      try {
        // Use a PreparedStatement instead of Statement
        PreparedStatement preparedStatement =
            connection.prepareStatement(
                query, ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);

        // Set the parameter value with the user input, ensuring it's safely treated as a value
